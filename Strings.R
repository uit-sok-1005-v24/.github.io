...
str_subset(fruit, pattern = "fruit") %>%
  str_split_fixed(pattern = " ", n = 2) #result is a matrix 
...
str_subset(fruit, pattern = "fruit") %>% 
  str_sub(1, 3) ##return the 1st 3 characters of each element
...
# regular expressions: is a pattern that describes a set of strings.
...
# match ia at the end of the country name, 
#preceded by one of the characters in the class
...
# country names that contain punctuation characters 
...
# change the quantifier from * to ? to require at most one 
#intervening character.In the strings that no longer match,
