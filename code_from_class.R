...
longdf %>%
  mutate(year = year(date)) %>% 
  filter(year %in% c(2000:2020)) %>% 
  filter(measurement %in% c("Lower_Troposphere","Mid_Troposphere")) %>% 
  group_by(measurement) %>%
  mutate(ma = zoo::rollmean(globe, 12, fill = NA, align = "right")) %>% 
  ggplot(aes(x = date, y = globe, color = measurement)) +
  geom_line(alpha=0.6, linetype = "dashed") +
...
longdf %>% 
  group_by(date) %>% 
  mutate(average_temp = mean(globe, na.rm = TRUE)) %>% 
  ungroup() %>% 
  ggplot(aes(x = date, y = globe, color = measurement)) +
  geom_line(alpha=0.6, linetype = "dashed") +
  geom_line(aes(x=date, y=average_temp)) +
  theme_bw()

test <-
  longdf %>% 
  group_by(date) %>% 
  summarise(average_temp = mean(globe, na.rm = TRUE)) %>% 
  rename(globe = average_temp) %>% 
  mutate(measurement = "average_temp")

longdf <- bind_rows(longdf, test)
